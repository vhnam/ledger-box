...
/**
 * Sync catalog map for server-side rendering (invite emails, etc.).
 * Do not import this from client app code — it pulls every locale into the bundle.
 * Prefer `loadMessages` / `DEFAULT_MESSAGES` from `@vhnam/utils/i18n` on the client.
 */
export const MESSAGES: Record<MessageLanguage, MessageCatalog> = {
  'en-US': enUS,
  'en-GB': enGB,
  'vi-VN': viVN,
  'ja-JP': jaJP,
