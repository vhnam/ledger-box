...
<!-- Generated by roast-my-design-system from a scan of vhnam/ledger-box, last rescanned 2026-08-08.
     Regenerate after big refactors: npx roast-my-design-system --rules -->

Follow these when writing or editing UI in this repo. Derived from a scan of this codebase, with real paths and usage counts.

**Colours and tokens** — Design tokens live in `packages/ui/src/styles/globals.css`. Reach for an existing token before inventing any value. Never hardcode colour values in components; add a token first if one is genuinely missing.

**Canonical components** — use these instead of writing new ones:

- `<Icon>` from `packages/ui/src/components/icon.tsx` (used 92x · props: name, icon)
- `<Button>` from `packages/ui/src/components/button.tsx` (used 69x · props: variant, size)
- `<FieldError>` from `packages/ui/src/components/field.tsx` (used 38x · props: errors)
- `<Field>` from `packages/ui/src/components/field.tsx` (used 37x · props: orientation)
- `<FieldLabel>` from `packages/ui/src/components/field.tsx` (used 29x)
- `<Spinner>` from `packages/ui/src/components/spinner.tsx` (used 27x)
- `<Input>` from `packages/ui/src/components/input.tsx` (used 18x · props: type)
- `<CardContent>` from `packages/ui/src/components/card.tsx` (used 13x)

**Components nobody imports** — the original scan's raw count (9) included compound-component internals it can't trace transitively (e.g. `Calendar`/`Popover` inside `date-picker.tsx`, `Label`/`Separator` inside `field.tsx`) — those are in active use. Only `Alert` and `Tabs` were genuinely dead (no caller anywhere, not even internally) and have been removed; a rescan now shows 1 remaining flag, likely another transitive false positive of the same shape. Before writing any new component, check this file for an existing one first; if you add a primitive that's not wired into the app yet, wire it in during the same change rather than leaving it to rot.

**Spacing and sizing** — Stay on the Tailwind spacing scale. No new arbitrary bracket values (`p-[13px]`, `text-[10px]`). The repeated `ring-[3px]` across five `packages/ui` primitives was replaced with the equivalent `ring-3` scale utility; a handful of one-off values remain (shadcn upstream boilerplate like `text-[0.8rem]`, or animation offsets in `sheet.tsx`) and are left as-is. If a bracket value repeats and a scale utility already covers it, use the scale utility instead of writing the bracket again.

**Styling discipline** — Never write `style={{ ... }}` for static values; styling belongs to classes and tokens where the system can see it. Before styling anything new, match how a neighbouring component does it.
...
**No en-dash or em-dash in source.** Do not write U+2013 (`–`) or U+2014 (`—`).
Use a plain ASCII hyphen `-` in UI copy, `defaultMessage`, catalogs, comments, and
tests (date ranges: `13/07/2026 - 19/07/2026`; empty fallback: `-`).
